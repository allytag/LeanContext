#!/usr/bin/env node
// leancontext — shared configuration resolver
//
// Resolution order for default mode:
//   1. LEANCONTEXT_DEFAULT_MODE environment variable
//   2. Config file defaultMode field:
//      - $XDG_CONFIG_HOME/leancontext/config.json (any platform, if set)
//      - ~/.config/leancontext/config.json (macOS / Linux fallback)
//      - %APPDATA%\leancontext\config.json (Windows fallback)
//   3. 'full'
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'leancontext');
  }
  ...
  return path.join(os.homedir(), '.config', 'leancontext');
}
...
function getDefaultMode() {
  ...
  const envMode = process.env.LEANCONTEXT_DEFAULT_MODE;
...
// flag path (~/.claude/.leancontext-active) with a symlink to clobber other files.
//
// Does NOT walk the full ancestor chain because legitimate platform-managed
// temp aliases and symlinked home directories can exist. The attack surface
// requires write access to the immediate parent, which is what we check.
//
...
function safeWriteFlag(flagPath, content) {
  try {
    ...
    const tempPath = path.join(flagDir, `.leancontext-active.${process.pid}.${Date.now()}`);
