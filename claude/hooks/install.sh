#!/bin/bash
# leancontext — one-command hook installer for Claude Code
# Installs: SessionStart hook (auto-load rules) + UserPromptSubmit hook (mode tracking)
# Usage: bash hooks/install.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/YOUR_GITHUB/LeanContext/main/hooks/install.sh)
...
if ! command -v node >/dev/null 2>&1; then
  echo "ERROR: 'node' is required to install the leancontext hooks (used to merge"
...
REPO_URL="https://raw.githubusercontent.com/YOUR_GITHUB/LeanContext/main/hooks"

HOOK_FILES=("package.json" "leancontext-config.js" "leancontext-activate.js" "leancontext-mode-tracker.js" "leancontext-statusline.sh")
...
if [ "$FORCE" -eq 0 ]; then
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ -f "$SETTINGS" ]; then
    if LEANCONTEXT_SETTINGS="$SETTINGS" node -e "
      const fs = require('fs');
      const settings = JSON.parse(fs.readFileSync(process.env.LEANCONTEXT_SETTINGS, 'utf8'));
      const hasLeanContextHook = (event) =>
        Array.isArray(settings.hooks?.[event]) &&
        settings.hooks[event].some(e =>
          e.hooks && e.hooks.some(h => h.command && h.command.includes('leancontext'))
        );
      process.exit(
        hasLeanContextHook('SessionStart') &&
        hasLeanContextHook('UserPromptSubmit') &&
        !!settings.statusLine
          ? 0
          : 1
      );
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ "$HOOKS_WIRED" -eq 1 ] && [ "$HAS_STATUSLINE" -eq 1 ]; then
    ALREADY_INSTALLED=1
    echo "LeanContext hooks already installed in $HOOKS_DIR"
...
if [ "$FORCE" -eq 1 ] && [ -f "$HOOKS_DIR/leancontext-activate.js" ]; then
  echo "Reinstalling leancontext hooks (--force)..."
else
  echo "Installing leancontext hooks..."
fi
...
chmod +x "$HOOKS_DIR/leancontext-statusline.sh"
...
LEANCONTEXT_SETTINGS="$SETTINGS" LEANCONTEXT_HOOKS_DIR="$HOOKS_DIR" node -e "
  const fs = require('fs');
  const settingsPath = process.env.LEANCONTEXT_SETTINGS;
  const hooksDir = process.env.LEANCONTEXT_HOOKS_DIR;
  const managedStatusLinePath = hooksDir + '/leancontext-statusline.sh';
  const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
  if (!settings.hooks) settings.hooks = {};

  // SessionStart — auto-load LeanContext rules
  ...
  if (!hasStart) {
    settings.hooks.SessionStart.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/leancontext-activate.js\"',
        timeout: 5,
        statusMessage: 'Loading LeanContext mode...'
      }]
    });
  }

  // UserPromptSubmit — track mode changes when user types /leancontext commands
  ...
  if (!hasPrompt) {
    settings.hooks.UserPromptSubmit.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/leancontext-mode-tracker.js\"',
        timeout: 5,
        statusMessage: 'Tracking LeanContext mode...'
      }]
    });
  }

  // Statusline — wire leancontext badge (report if skipped)
  if (!settings.statusLine) {
    ...
    if (cmd.includes(managedStatusLinePath)) {
      ...
      console.log('  NOTE: Existing statusline detected — leancontext badge NOT added.');
...
echo "  - SessionStart hook: auto-loads LeanContext rules every session"
echo "  - Mode tracker hook: updates statusline badge when you switch modes"
echo "    (/leancontext lite, /leancontext ultra, /leancontext-commit, etc.)"
echo "  - Statusline badge: shows [LEANCONTEXT] or [LEANCONTEXT:ULTRA] etc."
