# leancontext — one-command hook installer for Claude Code (Windows PowerShell)
...
#        irm https://raw.githubusercontent.com/YOUR_GITHUB/LeanContext/main/hooks/install.ps1 | iex
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    Write-Host "ERROR: 'node' is required to install the leancontext hooks (used to merge" -ForegroundColor Red
...
$RepoUrl = "https://raw.githubusercontent.com/YOUR_GITHUB/LeanContext/main/hooks"

$HookFiles = @("package.json", "leancontext-config.js", "leancontext-activate.js", "leancontext-mode-tracker.js", "leancontext-statusline.sh", "leancontext-statusline.ps1")
...
if (-not $Force) {
    ...
    if ($AllFilesPresent -and (Test-Path $Settings)) {
        try {
                ...
                foreach ($entry in $entries) {
                    if ($entry.hooks) {
                        foreach ($hookDef in $entry.hooks) {
                            if ($hookDef.command -and $hookDef.command.Contains("leancontext")) {
                                return $true
                            }
                        }
                    }
            ...
            $HooksWired = (& $hasLeanContextHook "SessionStart") -and (& $hasLeanContextHook "UserPromptSubmit")
    ...
    if ($AllFilesPresent -and $HooksWired -and $HasStatusLine) {
        Write-Host "LeanContext hooks already installed in $HooksDir"
...
if ($Force -and (Test-Path (Join-Path $HooksDir "leancontext-activate.js"))) {
    Write-Host "Reinstalling leancontext hooks (-Force)..."
} else {
    Write-Host "Installing leancontext hooks..."
}
...
$env:LEANCONTEXT_SETTINGS = $Settings -replace '\\', '/'
$env:LEANCONTEXT_HOOKS_DIR = $HooksDir -replace '\\', '/'

$nodeScript = @'
const fs = require('fs');
const settingsPath = process.env.LEANCONTEXT_SETTINGS;
const hooksDir = process.env.LEANCONTEXT_HOOKS_DIR;
const managedStatusLinePath = hooksDir + '/leancontext-statusline.ps1';
...
Write-Host "  - SessionStart hook: auto-loads LeanContext rules every session"
Write-Host "  - Mode tracker hook: updates statusline badge when you switch modes"
Write-Host "    (/leancontext lite, /leancontext ultra, /leancontext-commit, etc.)"
Write-Host "  - Statusline badge: shows [LEANCONTEXT] or [LEANCONTEXT:ULTRA] etc."
