...
def home_plugin_status(source_version: str | None) -> tuple[bool, str]:
    if not HOME_PLUGIN_PATH.exists():
        return False, f"missing home plugin path: {HOME_PLUGIN_PATH}"

    try:
        if HOME_PLUGIN_PATH.resolve() == PLUGIN_ROOT.resolve():
            return True, f"home plugin path linked: {HOME_PLUGIN_PATH}"
    except OSError:
        pass

    manifest = payload_manifest(HOME_PLUGIN_PATH)
    if not manifest:
        return False, f"home plugin path is not a LeanContext plugin payload: {HOME_PLUGIN_PATH}"
    if manifest.get("name") != PLUGIN_NAME:
        return False, f"home plugin path has wrong plugin name: {manifest.get('name')}"
    if source_version and str(manifest.get("version")) != source_version:
        return False, (
            f"home plugin copy version mismatch: {manifest.get('version')} != {source_version} "
            "(run install_codex_local.py to refresh it)"
        )

    return True, f"home plugin path copied: {HOME_PLUGIN_PATH}"
...
def main() -> int:
    ...
    home_ok, home_message = home_plugin_status(source_version)
    if home_ok:
        print(f"[OK] {home_message}")
    else:
        failures.append(home_message)
