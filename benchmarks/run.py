#!/usr/bin/env python3
"""Benchmark leancontext vs normal Claude output token counts."""
...
SKILL_PATH = REPO_DIR / "skills" / "leancontext" / "SKILL.md"
...
def run_benchmarks(client, model, prompts, leancontext_system, trials):
    results = []
    total = len(prompts)
    ...
    for i, prompt_entry in enumerate(prompts, 1):
        ...
        for mode, system in [("normal", NORMAL_SYSTEM), ("leancontext", leancontext_system)]:
...
def compute_stats(results):
    ...
    for entry in results:
        ...
        leancontext_medians = statistics.median(
            [t["output_tokens"] for t in entry["leancontext"]]
        )
        savings = 1 - (leancontext_medians / normal_medians) if normal_medians > 0 else 0
        ...
        rows.append(
            {
                ...
                "leancontext_median": int(leancontext_medians),
    ...
    avg_leancontext = round(statistics.mean([r["leancontext_median"] for r in rows]))
    ...
    return rows, {
        ...
        "avg_leancontext": avg_leancontext,
    }
...
def format_table(rows, summary):
    lines = [
        "| Task | Normal (tokens) | LeanContext (tokens) | Saved |",
        "|------|---------------:|----------------:|------:|",
    ]
    for r in rows:
        label = format_prompt_label(r["id"])
        lines.append(
            f"| {label} | {r['normal_median']} | {r['leancontext_median']} | {r['savings_pct']}% |"
        )
    lines.append(
        f"| **Average** | **{summary['avg_normal']}** | **{summary['avg_leancontext']}** | **{summary['avg_savings']}%** |"
    )
    lines.append("")
    lines.append(
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark leancontext vs normal Claude")
    ...
    leancontext_system = load_leancontext_system()
    ...
    results = run_benchmarks(client, args.model, prompts, leancontext_system, args.trials)
